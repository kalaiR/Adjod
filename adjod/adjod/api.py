...
ProductAPI = fxapi.RestView.create('productlist', Product, actions=['list'])
print "absentlistview", ProductAPI
...
class Product_RestAPI(fxapi.RestView):
    print "enter Product_RestAPI"
    name = 'productlist'
    model = Product
    actions = ['list']
    ...
    #   print "get_details"
    #   print "user", user
    #   productlist = Product.objects.get(user=user)
    #   print productlist
    
    @classmethod
    def action_list(cls, request, limit=None, pk=None, action='list'):
        print "enter action_list"
        result = super(Product_RestAPI, cls).action_list(request, limit, pk, action)
        ...
        # productlists = Product.objects.filter(title=title)
        # result['data'] = productlists
