...
(function( $ ) {
  ...
  var xhr_dlogin = $.get( "/dialog_login/", function( dialog_login ) {
  ...
  var xhr_options = $.get( "/options/", function( options ) {
    $( "body" ).append( options );
  });
  ...
  $.when( xhr_dlogin, xhr_toolbar, xhr_mchat, xhr_options ).done(function(r1, r2, r3, r4) {
    ...
    $( document ).on("click", "#chat-icon-close", function() {
      ...
      if( user_email && user_name){
        ...
        // if(document.forms["sign_in"].submit()){       
        // // main_chat_set_position(0);
        // open_chat_box ( 1, field1 );
        // }
      }
      }
      else{
    ...
    $( document ).on("click", ".user-status", function() {
      ...
      if ( id == "user-status-online" && now != "img/button_online.png" ) {
        status = "online";
        label  = i18n.connected;
      }

      else if ( id == "user-status-busy" && now != "img/busy.png" ) {
        status = "busy";
        label  = i18n.busy;
      }

      else if ( id == "user-status-offline" && now != "img/button_offline.png" ) {
        status = "offline";
        label  = i18n.offline;
      }
    ...
    function main_chat_user_new ( id, status, name ) {
      ...
      $( ".product_users" ).each( function( index, element ){
        ...
        if ($(this).text()==name){
        alert("enter if");
        // $(this).next('.contact_button').show();
        $( "#main-sort-chat" ).append( "<div id='user-" + id + "' class='user'><li class='" + status + "'><a href='#' style='text-decoration:none;' class='btn btn-default contact_button contact-button'> Chat Now </a></li></div>" );
        // $( "#chat_button" ).append( "<div id='user-" + id + "'><a href='#' style='text-decoration:none;' class='btn btn-default contact_button contact-button'>" + name + "</a></div>" );
        }
      });
    ...
    // var useremail = $.cookie('chat_email');
    // alert(useremail);
    // var userpass= $.cookie('chat_password');
    // alert(userpass);
    // socket_connect();
    // socket_handle();
