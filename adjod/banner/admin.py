...
class BannerPlanAdmin(admin.ModelAdmin):
    ...
    list_filter = ['page', 'position','price','plan_duration']
    search_fields = ['id', 'page','position','price','plan_duration']
    readonly_fields =['id']
    fieldsets = [
      ('Add BannerPlan', {
          'fields': [ 'id', 'page','position','price', 'plan_duration'] 
      }),
    ]
    list_per_page = 50
...
class PostBannerAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
      ('Add Banner', {
          'fields': [ 'id', 'user','banner','link','bannerplan', 'startdate','enddate','admin_status'] 
      }),
    ]
    list_per_page = 50
