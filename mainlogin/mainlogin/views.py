    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.is_active:
                ...
                # return HttpResponseRedirect('/start/?user_id=%s' %user_id)
...
#     return render(request, 'mainlogin/userpage.html', {}) 
      ...
      print "5"
      category=Category.objects.all()
      return render_to_response('mainlogin/userpage.html', {'category':category}, context_instance=RequestContext(request)
)
...
def start(request):
    ...
    return render_to_response('mainlogin/userpage.html',{'category':category,'path':path,'recentad':recentad,'product':product},
        context_instance=RequestContext(request)
    )
